...
`0.0.0` 只是本地工程候选包，不代表已发布到 nuget.org。M3 在官方 header manifest 上增加规范化确定性模型、ABI 验证的设备属性、显式 stream/event、异步内存/kernel 生命周期和双 stream VectorAdd sample。由于没有新的 Owner 云端授权，M3 GPU 门禁保持阻塞，不伪造 GPU 结果。
...
- M3 本地 managed gate：generator、ABI 契约、stream/event/async lease、sample build、package 和 DocFX 门禁已覆盖。
- M3 GPU-validated：阻塞，等待当次 Owner 授权的 Radeon Cloud 实例。
