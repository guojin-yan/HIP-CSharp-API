...
`eng/prepare-runtime.ps1` stages native assets outside Git under `runtimes/<rid>/native`. `eng/pack-runtime.ps1` invokes the same validator used by direct MSBuild packing; a property cannot bypass it. `-Candidate` requires an ignored clean-SHA attestation and emits an explicitly unverified package. Final mode requires the exact tracked M8.7 promotion receipt and lock, embeds the receipt, and rejects direct `dotnet pack`, partial, forged, or stale evidence with `HIPSHARP1001`. `eng/verify-runtime-package.ps1` audits the candidate/final ZIP allowlist, hashes, RID, licenses, SBOM, receipt, forbidden payload, managed assembly absence, repository commit, package size, and non-publishable authorization boundary.
