...
`categories.json` classifies the exported surface as formal or diagnostic and records sample-only and internal roots. A baseline update must be reviewed together with `docs/releases/1.0.0.md` and `docs/releases/1.0.0-readiness.md`; it is not a routine generated-file refresh. M8.9 promotion locks and `0.9.1` release records remain historical inputs and must not be rewritten for a `1.0.0` gate.
