...
`managed` rows map 109 ABI-manifest entries plus 130 reviewed high-level promotions to concrete unit-test sources. The original 100 entries retain `passed-historical` cloud status bound to the exact historical 0.x SHA. The nine 0.10.0 HIPRTC additions, M8.13 promotions, and M8.14 advanced interop promotions remain `not-tested` until their own exact-SHA Radeon Cloud workloads run. The 238 `raw-only-reviewed` rows remain explicit. No row upgrades a symbol export or historical result into a current function-level pass.
