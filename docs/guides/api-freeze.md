...
| Module and kernel | `HipModule` owns the native module; kernels are valid only while that module remains alive. Attributes and occupancy are typed value queries on module-owned `hipFunction_t`. Explicit-stream ordinary and cooperative submissions retain the module and pointer owners until completion; cooperative launch validates device capability and resident grid capacity without an ordinary-launch fallback. |
