...
M8.3 memory-pool and M8.4 explicit-graph APIs are currently managed-only validated. Their official-header signatures, ABI probe, and cloud symbol gate are updated, but no new Owner-authorized Linux symbol, Runtime, or GPU execution was performed; Windows support for these families remains static-only and unverified.
