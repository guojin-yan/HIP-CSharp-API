...
M8.3 memory-pool, M8.4 explicit-graph, M8.5 kernel occupancy/cooperative-launch, and M8.6 module-global APIs are currently managed-only validated. Their official-header signatures, ABI probe, and cloud symbol gate are updated, but no new Owner-authorized Linux symbol, Runtime, real module-symbol content, occupancy-result, or GPU execution was performed; Windows support for these families remains static-only and unverified.
