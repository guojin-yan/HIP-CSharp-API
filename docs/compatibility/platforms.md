...
M8.3 memory-pool APIs are currently managed-only validated. Their official-header signatures and cloud symbol gate are updated, but no new Owner-authorized Linux Runtime/GPU execution was performed; Windows memory-pool support remains static-only and unverified.
