...
Prior M1/M2 validation passed on an authorized Radeon Cloud Ubuntu 24.04.4, ROCm 7.2.1, HIP 7.2.53211, and `gfx1100` session. M4 carries 31 Runtime exports, explicit streams/events, async memory/kernel and lease checks into a local release-candidate audit, but has no new official-header or GPU result because the required Owner authorization and ROCm toolchain were not provided. Windows HIP SDK 7.2 loader paths are implemented but have not received AMD GPU execution. These are validation results for one environment, not support claims. The core package does not ship the driver or ROCm user-mode libraries.
