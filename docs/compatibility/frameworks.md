...
For the current `0.10.0` release, `eng/verify-public-api.ps1` compares the reviewed additive snapshot with every target assembly. All 15 TFMs must remain surface-identical. The separately retained `0.9.2` and `0.9.3` snapshots are not rewritten. The pack step also runs NuGet package validation.
