...
`cloud-test.sh` runs the managed build/test/package gate, the core package audit and clean consumers (PowerShell is required for `eng/verify-package.ps1`), verifies the 99 managed-manifest exports, and verifies the complete pinned-header model against the installed ROCm headers and libraries. ROCm 7.2.1 on Linux exports 458 of 459 Runtime declarations; `hipExternalMemoryGetMappedMipmappedArray` is the single explicit reviewed exception. All 18 HIPRTC declarations must be exported. The gate also compiles the schema 6 owner ABI probe, including M8.5 module function-attribute, occupancy, and cooperative-launch signatures/enums plus M8.4 graph layouts, and executes DeviceInfo, MemoryCopy, HIPRTC VectorAdd/negative compile, stream/event, and advanced API paths. Real execution of newly added managed paths still requires a separately authorized session and workload. The reliability gate compares every lane with the CPU without emitting timing or a performance claim.
