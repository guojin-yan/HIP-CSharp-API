...
`cloud-test.sh` runs the managed build/test/package gate, verifies the required Runtime and HIPRTC exports separately, compiles the M2 ABI probe against both installed HIP headers, and executes DeviceInfo, MemoryCopy, five VectorAdd lengths for 20 repetitions each, and an expected HIPRTC compiler failure. Raw outputs stay under ignored `artifacts/radeon-cloud/`; copy and review them into the external `Radeon_Cloud/records/<session>/` structure before treating the run as evidence. `env-report.sh` suppresses the cloud hostname and identifying GPU fields, and it does not request the GPU unique identifier.
