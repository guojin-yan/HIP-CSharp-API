...
`cloud-test.sh` runs the managed build/test/package gate, the core package audit and clean consumers (PowerShell is required for `eng/verify-package.ps1`), verifies the 55 lifecycle-owner exports, and verifies the complete pinned-header model against the installed ROCm headers and libraries. ROCm 7.2.1 on Linux exports 458 of 459 Runtime declarations; `hipExternalMemoryGetMappedMipmappedArray` is the single explicit reviewed exception. All 18 HIPRTC declarations must be exported. The gate also compiles the schema 3 owner ABI probe and executes DeviceInfo, MemoryCopy, HIPRTC VectorAdd/negative compile, stream/event, and advanced API paths. It then calls `cloud-stress.sh`, whose default reliability profile submits 4,194,304-float vectors to four streams for ten allocation/release rounds, keeps at most 192 MiB of device buffers in flight, and compares every lane with the CPU without emitting timing or a performance claim.
