...
`cloud-test.sh` runs the managed build/test/package gate, the core package audit and clean consumers (PowerShell is required for `eng/verify-package.ps1`), verifies 46 Runtime and 9 HIPRTC exports, compiles the schema 3 ABI probe against both installed HIP headers, and executes DeviceInfo, MemoryCopy, HIPRTC VectorAdd/negative compile, stream/event, and advanced API paths. It then calls `cloud-stress.sh`, whose default reliability profile submits 4,194,304-float vectors to four streams for ten allocation/release rounds, keeps at most 192 MiB of device buffers in flight, and compares every lane with the CPU without emitting timing or a performance claim.
