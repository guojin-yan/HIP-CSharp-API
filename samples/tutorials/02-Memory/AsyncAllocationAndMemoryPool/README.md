# AsyncAllocationAndMemoryPool

<p align="center"><strong>English</strong> | <a href="README.zh-CN.md">简体中文</a></p>

`AsyncAllocationAndMemoryPool` is the 02-Memory tutorial case. Demonstrates stream-ordered allocation, asynchronous release, a custom memory pool, and owner lifetime ordering.

## What This Teaches

This case focuses on HipAsyncAllocator, HipMemoryPool, stream ordering, and deferred disposal.
...
```bash
dotnet run --project samples/tutorials/02-Memory/AsyncAllocationAndMemoryPool/AsyncAllocationAndMemoryPool.csproj --configuration Release
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`AsyncAllocationAndMemoryPool` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\02-Memory\AsyncAllocationAndMemoryPool\AsyncAllocationAndMemoryPool.csproj --locked-mode
dotnet build .\samples\tutorials\02-Memory\AsyncAllocationAndMemoryPool\AsyncAllocationAndMemoryPool.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\02-Memory\AsyncAllocationAndMemoryPool\AsyncAllocationAndMemoryPool.csproj --configuration Release --no-build --
```
...
```text
Async allocation and memory-pool round trip passed.
```
...
The cloud log reports a successful async allocation and memory-pool round trip.
...
The case log is `logs/AsyncAllocationAndMemoryPool.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `AsyncAllocationAndMemoryPool.csproj` | .NET target and local HIPSharp project reference |
