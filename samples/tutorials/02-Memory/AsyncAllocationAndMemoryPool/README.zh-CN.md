# AsyncAllocationAndMemoryPool / 异步分配与内存池

<p align="center"><a href="README.md">English</a> | <strong>简体中文</strong></p>

`AsyncAllocationAndMemoryPool` 是 02-Memory 模块中的教程案例。演示 Stream-ordered allocation、异步释放、自定义内存池和 Owner 生命周期顺序。

## 本案例学习什么

本案例重点演示：HipAsyncAllocator、HipMemoryPool、Stream 顺序和延迟释放。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `AsyncAllocationAndMemoryPool` 状态是本案例的
权威云端结果。
...
本案例日志为 `logs/AsyncAllocationAndMemoryPool.log`；`results.csv` 保存 20 个教程的状态和退出码。
...
| `AsyncAllocationAndMemoryPool.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
| `packages.lock.json` | 锁定的依赖图 |
| `../../run-cloud-verification.sh` | 云端架构检测、构建矩阵、运行和证据保存 |

建议先阅读所在模块说明：[`02-Memory`](../README.zh-CN.md)。
