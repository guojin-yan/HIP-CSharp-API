# LinearMemoryCopy / Linear memory copy

<p align="center"><strong>English</strong> | <a href="README.zh-CN.md">简体中文</a></p>

`LinearMemoryCopy` is the 02-Memory tutorial case. Allocates device buffers and verifies a 256-byte H2D, D2D, and D2H round trip.

## What This Teaches

This case focuses on HipDeviceMemory ownership, synchronous copies, CPU comparison, and release order.
...
```bash
dotnet run --project samples/tutorials/02-Memory/LinearMemoryCopy/LinearMemoryCopy.csproj --configuration Release
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`LinearMemoryCopy` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\02-Memory\LinearMemoryCopy\LinearMemoryCopy.csproj --locked-mode
dotnet build .\samples\tutorials\02-Memory\LinearMemoryCopy\LinearMemoryCopy.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\02-Memory\LinearMemoryCopy\LinearMemoryCopy.csproj --configuration Release --no-build --
```
...
```text
HIP H2D/D2D/D2H memory round trip passed.
```
...
The cloud log reports a successful H2D/D2D/D2H round trip.
...
The case log is `logs/LinearMemoryCopy.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `LinearMemoryCopy.csproj` | .NET target and local HIPSharp project reference |
