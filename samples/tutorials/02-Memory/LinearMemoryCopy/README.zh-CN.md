...
`LinearMemoryCopy` 是 02-Memory 模块中的教程案例。分配设备缓冲区，验证 256 字节 H2D、D2D 和 D2H 往返复制。

## 本案例学习什么

本案例重点演示：HipDeviceMemory 所有权、同步 Copy、CPU 比较和释放顺序。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `LinearMemoryCopy` 状态是本案例的
权威云端结果。
...
| `LinearMemoryCopy.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
