...
`ManagedMemory` 是 02-Memory 模块中的教程案例。演示 HipManagedMemory 可见性、Advice、Prefetch 和同步。

## 本案例学习什么

本案例重点演示：Managed 分配、内存 Advice、Prefetch 和 CPU 可见性校验。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `ManagedMemory` 状态是本案例的
权威云端结果。
...
本案例日志为 `logs/ManagedMemory.log`；`results.csv` 保存 20 个教程的状态和退出码。
...
| `ManagedMemory.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
