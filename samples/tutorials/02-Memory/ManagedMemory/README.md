...
`ManagedMemory` is the 02-Memory tutorial case. Demonstrates HipManagedMemory visibility, usage advice, prefetch, and synchronization.

## What This Teaches

This case focuses on Managed allocation, memory advice, prefetch, and CPU-visible validation.
...
```bash
dotnet run --project samples/tutorials/02-Memory/ManagedMemory/ManagedMemory.csproj --configuration Release
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`ManagedMemory` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\02-Memory\ManagedMemory\ManagedMemory.csproj --locked-mode
dotnet build .\samples\tutorials\02-Memory\ManagedMemory\ManagedMemory.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\02-Memory\ManagedMemory\ManagedMemory.csproj --configuration Release --no-build --
```
...
```text
Managed memory round trip passed.
```
...
The cloud log reports a successful managed memory round trip.
...
The case log is `logs/ManagedMemory.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `ManagedMemory.csproj` | .NET target and local HIPSharp project reference |
