...
`VirtualMemory` 是 02-Memory 模块中的教程案例。预留虚拟地址、分配物理内存、建立映射、设置访问策略并按顺序释放。

## 本案例学习什么

本案例重点演示：虚拟地址预留、物理分配、映射、访问描述符和清理。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `VirtualMemory` 状态是本案例的
权威云端结果。
...
| `VirtualMemory.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
