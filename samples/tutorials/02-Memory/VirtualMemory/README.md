...
`VirtualMemory` is the 02-Memory tutorial case. Reserves virtual addresses, allocates physical memory, maps it, sets access policy, and releases in order.

## What This Teaches

This case focuses on Virtual address reservation, physical allocation, mapping, access descriptors, and cleanup.
...
```bash
dotnet run --project samples/tutorials/02-Memory/VirtualMemory/VirtualMemory.csproj --configuration Release
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`VirtualMemory` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\02-Memory\VirtualMemory\VirtualMemory.csproj --locked-mode
dotnet build .\samples\tutorials\02-Memory\VirtualMemory\VirtualMemory.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\02-Memory\VirtualMemory\VirtualMemory.csproj --configuration Release --no-build --
```
...
```text
Virtual memory reserve/map/access passed.
```
...
The cloud log reports a successful reserve/map/access path.
...
The case log is `logs/VirtualMemory.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `VirtualMemory.csproj` | .NET target and local HIPSharp project reference |
