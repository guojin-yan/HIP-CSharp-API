# PinnedHostMemory / 锁页主机内存

<p align="center"><a href="README.md">English</a> | <strong>简体中文</strong></p>

`PinnedHostMemory` 是 02-Memory 模块中的教程案例。管理 HipPinnedMemory，并验证经过 HIP 锁页主机内存的主机往返。

## 本案例学习什么

本案例重点演示：锁页主机内存分配、Owner 生命周期和主机可见 Copy。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `PinnedHostMemory` 状态是本案例的
权威云端结果。
...
本案例日志为 `logs/PinnedHostMemory.log`；`results.csv` 保存 20 个教程的状态和退出码。
...
| `PinnedHostMemory.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
