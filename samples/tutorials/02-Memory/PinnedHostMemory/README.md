...
`PinnedHostMemory` is the 02-Memory tutorial case. Owns HipPinnedMemory and verifies a host round trip through HIP pinned memory.

## What This Teaches

This case focuses on Pinned host allocation, owner lifetime, and host-visible copies.
...
```bash
dotnet run --project samples/tutorials/02-Memory/PinnedHostMemory/PinnedHostMemory.csproj --configuration Release
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`PinnedHostMemory` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\02-Memory\PinnedHostMemory\PinnedHostMemory.csproj --locked-mode
dotnet build .\samples\tutorials\02-Memory\PinnedHostMemory\PinnedHostMemory.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\02-Memory\PinnedHostMemory\PinnedHostMemory.csproj --configuration Release --no-build --
```
...
```text
Pinned host memory round trip passed.
```
...
The cloud log reports a successful pinned host memory round trip.
...
The case log is `logs/PinnedHostMemory.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `PinnedHostMemory.csproj` | .NET target and local HIPSharp project reference |
