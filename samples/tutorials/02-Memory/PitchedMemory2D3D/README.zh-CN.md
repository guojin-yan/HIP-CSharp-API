# PitchedMemory2D3D / Pitched 2D/3D 内存

<p align="center"><a href="README.md">English</a> | <strong>简体中文</strong></p>

`PitchedMemory2D3D` 是 02-Memory 模块中的教程案例。创建类型化 2D/3D 分配，检查物理行 pitch，并复制紧凑排列的主机数据。

## 本案例学习什么

本案例重点演示：Pitched extent、物理 pitch 与逻辑宽度的差异，以及紧凑主机 Copy。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `PitchedMemory2D3D` 状态是本案例的
权威云端结果。
...
云端日志记录 2D/3D extent、256 字节 pitch，且往返通过。

完整证据目录位于
[`Radeon_Cloud/records/20260818-161709-tutorials`](../../../../../Radeon_Cloud/records/20260818-161709-tutorials)。
本案例日志为 `logs/PitchedMemory2D3D.log`；`results.csv` 保存 20 个教程的状态和退出码。
...
| `PitchedMemory2D3D.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
