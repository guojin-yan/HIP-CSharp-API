...
`PitchedMemory2D3D` is the 02-Memory tutorial case. Creates typed 2D and 3D allocations, inspects physical row pitch, and copies tightly packed host data.

## What This Teaches

This case focuses on Pitched extents, physical pitch versus logical width, and packed host copies.
...
```bash
dotnet run --project samples/tutorials/02-Memory/PitchedMemory2D3D/PitchedMemory2D3D.csproj --configuration Release
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`PitchedMemory2D3D` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\02-Memory\PitchedMemory2D3D\PitchedMemory2D3D.csproj --locked-mode
dotnet build .\samples\tutorials\02-Memory\PitchedMemory2D3D\PitchedMemory2D3D.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\02-Memory\PitchedMemory2D3D\PitchedMemory2D3D.csproj --configuration Release --no-build --
```
...
```text
2D extent=4x3, pitch=256 bytes; 3D extent=4x3x2, pitch=256 bytes; Pitched 2D/3D memory round trip passed.
```
...
The cloud log records 2D/3D extents, 256-byte pitch, and a passing round trip.
...
The case log is `logs/PitchedMemory2D3D.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `PitchedMemory2D3D.csproj` | .NET target and local HIPSharp project reference |
