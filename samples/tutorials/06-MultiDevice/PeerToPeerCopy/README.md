# PeerToPeerCopy / peer-to-peer copy

<p align="center"><strong>English</strong> | <a href="README.zh-CN.md">简体中文</a></p>

`PeerToPeerCopy` is the 06-MultiDevice tutorial case. Checks a directed device pair, enables peer access, copies four bytes from device 1 to device 0, and verifies the result.

## What This Teaches

This case focuses on HipRuntime.GetDevices, CanAccessPeer, MakeCurrent, HipPeerAccess, and capability-gated skip.
...
```bash
dotnet run --project samples/tutorials/06-MultiDevice/PeerToPeerCopy/PeerToPeerCopy.csproj --configuration Release
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`PeerToPeerCopy` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\06-MultiDevice\PeerToPeerCopy\PeerToPeerCopy.csproj --locked-mode
dotnet build .\samples\tutorials\06-MultiDevice\PeerToPeerCopy\PeerToPeerCopy.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\06-MultiDevice\PeerToPeerCopy\PeerToPeerCopy.csproj --configuration Release --no-build --
```
...
```text
Skipped: at least two HIP devices are required.
```
...
The cloud instance exposes one GPU, so the program correctly returned a controlled skip.
...
The case log is `logs/PeerToPeerCopy.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `PeerToPeerCopy.csproj` | .NET target and local HIPSharp project reference |
