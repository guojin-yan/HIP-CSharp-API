...
`PeerToPeerCopy` 是 06-MultiDevice 模块中的教程案例。检查有方向的设备对，启用 Peer Access，将四个字节从设备 1 复制到设备 0 并验证。

## 本案例学习什么

本案例重点演示：HipRuntime.GetDevices、CanAccessPeer、MakeCurrent、HipPeerAccess 和能力门控跳过。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `PeerToPeerCopy` 状态是本案例的
权威云端结果。
...
| `PeerToPeerCopy.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
| `packages.lock.json` | 锁定的依赖图 |
| `../../run-cloud-verification.sh` | 云端架构检测、构建矩阵、运行和证据保存 |

建议先阅读所在模块说明：[`06-MultiDevice`](../README.zh-CN.md)。
