...
`LoaderDiagnostics` is the 01-RuntimeDevice tutorial case. Shows the native amdhip64 loading path and preserves diagnostic attempts when the loader cannot be found.

## What This Teaches

This case focuses on HipRuntime loading, HipLibraryLoadException, and loader diagnostics.
...
```bash
dotnet run --project samples/tutorials/01-RuntimeDevice/LoaderDiagnostics/LoaderDiagnostics.csproj --configuration Release
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`LoaderDiagnostics` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\01-RuntimeDevice\LoaderDiagnostics\LoaderDiagnostics.csproj --locked-mode
dotnet build .\samples\tutorials\01-RuntimeDevice\LoaderDiagnostics\LoaderDiagnostics.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\01-RuntimeDevice\LoaderDiagnostics\LoaderDiagnostics.csproj --configuration Release --no-build --
```
...
```text
HIP Runtime loaded successfully.
```
...
The cloud host loaded the HIP Runtime successfully.
...
The case log is `logs/LoaderDiagnostics.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `LoaderDiagnostics.csproj` | .NET target and local HIPSharp project reference |
