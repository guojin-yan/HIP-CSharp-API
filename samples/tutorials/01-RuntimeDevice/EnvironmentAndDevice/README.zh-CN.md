# EnvironmentAndDevice / 环境与设备

<p align="center"><a href="README.md">English</a> | <strong>简体中文</strong></p>

`EnvironmentAndDevice` 是 01-RuntimeDevice 模块中的教程案例。初始化 HIP，输出 Runtime/Driver 版本，并枚举设备序号与名称。

## 本案例学习什么

本案例重点演示：HipRuntime.Initialize、GetVersionInfo、GetDevices 和 HipDevice 属性。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `EnvironmentAndDevice` 状态是本案例的
权威云端结果。
...
云端日志输出 Runtime 7.2.53211、Driver 7.2.53211，并枚举出一张 AMD Radeon Graphics。

完整证据目录位于
[`Radeon_Cloud/records/20260818-161709-tutorials`](../../../../../Radeon_Cloud/records/20260818-161709-tutorials)。
本案例日志为 `logs/EnvironmentAndDevice.log`；`results.csv` 保存 20 个教程的状态和退出码。
...
| `EnvironmentAndDevice.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
| `packages.lock.json` | 锁定的依赖图 |
| `../../run-cloud-verification.sh` | 云端架构检测、构建矩阵、运行和证据保存 |

建议先阅读所在模块说明：[`01-RuntimeDevice`](../README.zh-CN.md)。
