# EnvironmentAndDevice

<p align="center"><strong>English</strong> | <a href="README.zh-CN.md">简体中文</a></p>

`EnvironmentAndDevice` is the 01-RuntimeDevice tutorial case. Initializes HIP, reports runtime and driver versions, and enumerates device ordinals and names.

## What This Teaches

This case focuses on HipRuntime.Initialize, GetVersionInfo, GetDevices, and HipDevice attributes.
...
```bash
dotnet run --project samples/tutorials/01-RuntimeDevice/EnvironmentAndDevice/EnvironmentAndDevice.csproj --configuration Release
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`EnvironmentAndDevice` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\01-RuntimeDevice\EnvironmentAndDevice\EnvironmentAndDevice.csproj --locked-mode
dotnet build .\samples\tutorials\01-RuntimeDevice\EnvironmentAndDevice\EnvironmentAndDevice.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\01-RuntimeDevice\EnvironmentAndDevice\EnvironmentAndDevice.csproj --configuration Release --no-build --
```
...
```text
HIP Runtime: 7.2.53211; HIP Driver: 7.2.53211; 0: AMD Radeon Graphics
```
...
The cloud log reports runtime 7.2.53211, driver 7.2.53211, and one AMD Radeon Graphics device.
...
The case log is `logs/EnvironmentAndDevice.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `EnvironmentAndDevice.csproj` | .NET target and local HIPSharp project reference |
