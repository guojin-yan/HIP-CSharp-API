...
`ArrayTextureSurface` 是 07-DataObjects 模块中的教程案例。创建二维 HIP Array，验证主机往返，并在同一 Array 上创建 Texture 和 Surface Owner。

## 本案例学习什么

本案例重点演示：Array 分配、Array Copy、Texture/Surface 所有权和确定性释放。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `ArrayTextureSurface` 状态是本案例的
权威云端结果。
...
云端日志报告 Array/Texture/Surface 生命周期通过。

完整证据目录位于
[`Radeon_Cloud/records/20260818-161709-tutorials`](../../../../../Radeon_Cloud/records/20260818-161709-tutorials)。
本案例日志为 `logs/ArrayTextureSurface.log`；`results.csv` 保存 20 个教程的状态和退出码。
...
| `ArrayTextureSurface.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
| `packages.lock.json` | 锁定的依赖图 |
| `../../run-cloud-verification.sh` | 云端架构检测、构建矩阵、运行和证据保存 |

建议先阅读所在模块说明：[`07-DataObjects`](../README.zh-CN.md)。
