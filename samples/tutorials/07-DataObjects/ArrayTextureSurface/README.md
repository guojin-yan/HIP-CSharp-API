# ArrayTextureSurface / array, texture, and surface

<p align="center"><strong>English</strong> | <a href="README.zh-CN.md">简体中文</a></p>

`ArrayTextureSurface` is the 07-DataObjects tutorial case. Creates a two-dimensional HIP array, verifies a host round trip, and creates texture and surface owners over the same array.

## What This Teaches

This case focuses on Array allocation, array copies, texture/surface ownership, and deterministic disposal.
...
```bash
dotnet run --project samples/tutorials/07-DataObjects/ArrayTextureSurface/ArrayTextureSurface.csproj --configuration Release
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`ArrayTextureSurface` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\07-DataObjects\ArrayTextureSurface\ArrayTextureSurface.csproj --locked-mode
dotnet build .\samples\tutorials\07-DataObjects\ArrayTextureSurface\ArrayTextureSurface.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\07-DataObjects\ArrayTextureSurface\ArrayTextureSurface.csproj --configuration Release --no-build --
```
...
```text
Array/texture/surface lifecycle passed.
```
...
The cloud log reports a passing array/texture/surface lifecycle.
...
The case log is `logs/ArrayTextureSurface.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `ArrayTextureSurface.csproj` | .NET target and local HIPSharp project reference |
