...
`ModuleGlobals` is the 04-Kernel tutorial case. Compiles a module-owned device global and demonstrates the typed borrowed global view with a round trip.

## What This Teaches

This case focuses on HipModuleGlobal<T>, module symbol lookup, CopyFrom/CopyTo, and symbol lifetime.
...
```bash
dotnet run --project samples/tutorials/04-Kernel/ModuleGlobals/ModuleGlobals.csproj --configuration Release -- gfx1100
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`ModuleGlobals` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\04-Kernel\ModuleGlobals\ModuleGlobals.csproj --locked-mode
dotnet build .\samples\tutorials\04-Kernel\ModuleGlobals\ModuleGlobals.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\04-Kernel\ModuleGlobals\ModuleGlobals.csproj --configuration Release --no-build -- gfx1100
```
...
```text
Module global round trip passed.
```
...
The cloud run passed after defining the device global symbol with an initializer so HIPRTC exports it.
...
The case log is `logs/ModuleGlobals.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `ModuleGlobals.csproj` | .NET target and local HIPSharp project reference |
