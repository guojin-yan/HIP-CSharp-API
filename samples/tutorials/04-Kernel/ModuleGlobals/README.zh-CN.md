# ModuleGlobals / Module 全局变量

<p align="center"><a href="README.md">English</a> | <strong>简体中文</strong></p>

`ModuleGlobals` 是 04-Kernel 模块中的教程案例。编译 Module 所有的设备全局变量，并通过类型化借用视图完成往返。

## 本案例学习什么

本案例重点演示：HipModuleGlobal<T>、Module 符号查找、CopyFrom/CopyTo 和符号生命周期。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `ModuleGlobals` 状态是本案例的
权威云端结果。
...
修正设备全局变量定义并初始化以确保 HIPRTC 导出符号后，云端往返通过。

完整证据目录位于
[`Radeon_Cloud/records/20260818-161709-tutorials`](../../../../../Radeon_Cloud/records/20260818-161709-tutorials)。
本案例日志为 `logs/ModuleGlobals.log`；`results.csv` 保存 20 个教程的状态和退出码。
...
| `ModuleGlobals.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
