...
`KernelOccupancy` is the 04-Kernel tutorial case. Compiles a tiny kernel and queries occupancy plus the suggested launch plan for the target architecture.

## What This Teaches

This case focuses on HipKernel.GetOccupancy, HipOccupancyInfo, and HipOccupancyPlan.
...
```bash
dotnet run --project samples/tutorials/04-Kernel/KernelOccupancy/KernelOccupancy.csproj --configuration Release -- gfx1100
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`KernelOccupancy` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\04-Kernel\KernelOccupancy\KernelOccupancy.csproj --locked-mode
dotnet build .\samples\tutorials\04-Kernel\KernelOccupancy\KernelOccupancy.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\04-Kernel\KernelOccupancy\KernelOccupancy.csproj --configuration Release --no-build -- gfx1100
```
...
```text
Active blocks per multiprocessor: 384; Suggested block size: 1024; minimum grid: 96
```
...
The cloud log returned occupancy and launch-plan values.
...
The case log is `logs/KernelOccupancy.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `KernelOccupancy.csproj` | .NET target and local HIPSharp project reference |
