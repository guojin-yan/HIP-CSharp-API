# HipRtcVectorAdd / HIPRTC VectorAdd

<p align="center"><a href="README.md">English</a> | <strong>简体中文</strong></p>

`HipRtcVectorAdd` 是 04-Kernel 模块中的教程案例。在内存中编译 HIP C++ VectorAdd Kernel，加载 code object、启动 Kernel，并在 20 次重复后逐元素校验。

## 本案例学习什么

本案例重点演示：HIPRTC 源码、架构专用编译、Module/Kernel 加载、设备缓冲区、参数和 CPU 校验。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `HipRtcVectorAdd` 状态是本案例的
权威云端结果。
...
云端日志报告 HIPRTC 9.0、4960 字节 code object，且 20 次 VectorAdd 重复通过。

完整证据目录位于
[`Radeon_Cloud/records/20260818-161709-tutorials`](../../../../../Radeon_Cloud/records/20260818-161709-tutorials)。
本案例日志为 `logs/HipRtcVectorAdd.log`；`results.csv` 保存 20 个教程的状态和退出码。
...
| `HipRtcVectorAdd.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
| `packages.lock.json` | 锁定的依赖图 |
| `../../run-cloud-verification.sh` | 云端架构检测、构建矩阵、运行和证据保存 |

建议先阅读所在模块说明：[`04-Kernel`](../README.zh-CN.md)。
