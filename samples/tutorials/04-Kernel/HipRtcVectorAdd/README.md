...
`HipRtcVectorAdd` is the 04-Kernel tutorial case. Compiles an in-memory HIP C++ VectorAdd kernel, loads the code object, launches it, and checks every element after 20 repetitions.

## What This Teaches

This case focuses on HIPRTC source, architecture-specific compilation, module/kernel loading, device buffers, arguments, and CPU verification.
...
```bash
dotnet run --project samples/tutorials/04-Kernel/HipRtcVectorAdd/HipRtcVectorAdd.csproj --configuration Release -- --arch gfx1100 --length 1000 --repeat 20
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`HipRtcVectorAdd` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\04-Kernel\HipRtcVectorAdd\HipRtcVectorAdd.csproj --locked-mode
dotnet build .\samples\tutorials\04-Kernel\HipRtcVectorAdd\HipRtcVectorAdd.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\04-Kernel\HipRtcVectorAdd\HipRtcVectorAdd.csproj --configuration Release --no-build -- --arch gfx1100 --length 1000 --repeat 20
```
...
```text
HIPRTC version: 9.0; Code size: 4960; VectorAdd passed: length=1000; repeat=20; failureIndex=-1
```
...
The cloud log reports HIPRTC 9.0, a 4960-byte code object, and 20 successful VectorAdd repetitions.
...
The case log is `logs/HipRtcVectorAdd.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `HipRtcVectorAdd.csproj` | .NET target and local HIPSharp project reference |
