...
private const string LinkerKernelSource = @"
template <typename T>
...
private static float[] CreateInput(int length, int modulus, int multiplier)
{
...
private static int FindFailure(float[] a, float[] b, float[] actual)
{
...
private static int RunProgramLinkerValidation(HipRtc rtc, Options options, string source)
{
...
private static long RunLinkedCode(HipRuntime runtime, byte[] codeObject, string kernelName, int length, int repeat)
{
...
private static void ExpectRtcResult(HipRtcResult expected, Action action, string name, List<string> negatives)
{
    try
    {
        action();
...
private static void ExpectException<TException>(Action action, string name, List<string> negatives)
