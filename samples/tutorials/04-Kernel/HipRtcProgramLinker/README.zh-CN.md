# HipRtcProgramLinker / HIPRTC 程序链接

<p align="center"><a href="README.md">English</a> | <strong>简体中文</strong></p>

`HipRtcProgramLinker` 是 04-Kernel 模块中的教程案例。编译 LLVM bitcode，加入内存中的链接输入，并复制完成后的 code object。

## 本案例学习什么

本案例重点演示：HipRtcProgram.CompileToBitcode、HipRtcLinker、链接输入类型和 code object 所有权。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `HipRtcProgramLinker` 状态是本案例的
权威云端结果。
...
云端日志生成了 4088 字节的链接 code object。

完整证据目录位于
[`Radeon_Cloud/records/20260818-161709-tutorials`](../../../../../Radeon_Cloud/records/20260818-161709-tutorials)。
本案例日志为 `logs/HipRtcProgramLinker.log`；`results.csv` 保存 20 个教程的状态和退出码。
...
| `HipRtcProgramLinker.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
