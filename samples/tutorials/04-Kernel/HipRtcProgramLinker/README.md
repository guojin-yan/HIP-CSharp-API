...
`HipRtcProgramLinker` is the 04-Kernel tutorial case. Compiles LLVM bitcode, adds an in-memory linker input, and copies the completed code object.

## What This Teaches

This case focuses on HipRtcProgram.CompileToBitcode, HipRtcLinker, linker input types, and code-object ownership.
...
```bash
dotnet run --project samples/tutorials/04-Kernel/HipRtcProgramLinker/HipRtcProgramLinker.csproj --configuration Release -- gfx1100
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`HipRtcProgramLinker` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\04-Kernel\HipRtcProgramLinker\HipRtcProgramLinker.csproj --locked-mode
dotnet build .\samples\tutorials\04-Kernel\HipRtcProgramLinker\HipRtcProgramLinker.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\04-Kernel\HipRtcProgramLinker\HipRtcProgramLinker.csproj --configuration Release --no-build -- gfx1100
```
...
The cloud log produced a 4088-byte linked code object.
...
The case log is `logs/HipRtcProgramLinker.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `HipRtcProgramLinker.csproj` | .NET target and local HIPSharp project reference |
