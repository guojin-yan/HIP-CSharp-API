...
`PrecompiledModule` is the 04-Kernel tutorial case. Loads an external .hsaco or compatible code object and resolves a kernel without HIPRTC.

## What This Teaches

This case focuses on HipRuntime.LoadModule, HipKernel lookup, and external code-object deployment.
...
```bash
dotnet run --project samples/tutorials/04-Kernel/PrecompiledModule/PrecompiledModule.csproj --configuration Release -- PATH_TO_HSACO VectorAdd
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`PrecompiledModule` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\04-Kernel\PrecompiledModule\PrecompiledModule.csproj --locked-mode
dotnet build .\samples\tutorials\04-Kernel\PrecompiledModule\PrecompiledModule.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\04-Kernel\PrecompiledModule\PrecompiledModule.csproj --configuration Release --no-build -- PATH_TO_HSACO VectorAdd
```
...
```text
Usage: dotnet run --project ... -- <path-to-code-object> [kernel-name]
```

## Cloud Evidence

Cloud status: **Usage-only**.

The cloud matrix recorded the usage path because no portable .hsaco was supplied. Set HIPSHARP_PRECOMPILED_CODE_OBJECT to validate loading.
...
The case log is `logs/PrecompiledModule.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `PrecompiledModule.csproj` | .NET target and local HIPSharp project reference |
