# PrecompiledModule / 预编译 Module

<p align="center"><a href="README.md">English</a> | <strong>简体中文</strong></p>

`PrecompiledModule` 是 04-Kernel 模块中的教程案例。加载外部 .hsaco 或兼容 code object，在不使用 HIPRTC 的情况下解析 Kernel。

## 本案例学习什么

本案例重点演示：HipRuntime.LoadModule、HipKernel 查找和外部 code object 部署。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `PrecompiledModule` 状态是本案例的
权威云端结果。
...
由于没有提供可移植 .hsaco，云端矩阵记录了入口用法路径；设置 HIPSHARP_PRECOMPILED_CODE_OBJECT 后可验证加载。

完整证据目录位于
[`Radeon_Cloud/records/20260818-161709-tutorials`](../../../../../Radeon_Cloud/records/20260818-161709-tutorials)。
本案例日志为 `logs/PrecompiledModule.log`；`results.csv` 保存 20 个教程的状态和退出码。
...
| `PrecompiledModule.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
