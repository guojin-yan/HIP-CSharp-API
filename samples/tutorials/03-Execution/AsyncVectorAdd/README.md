# AsyncVectorAdd / asynchronous VectorAdd

<p align="center"><strong>English</strong> | <a href="README.zh-CN.md">简体中文</a></p>

`AsyncVectorAdd` is the 03-Execution tutorial case. Compiles VectorAdd with HIPRTC, uses two non-blocking streams and events, validates five lengths, captures an expected error, and repeats lifecycle operations 100 times.

## What This Teaches

This case focuses on HIPRTC, async H2D/kernel/D2H ordering, event synchronization, CPU comparison, and deterministic disposal.
...
```bash
dotnet run --project samples/tutorials/03-Execution/AsyncVectorAdd/AsyncVectorAdd.csproj --configuration Release -- --arch gfx1100
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`AsyncVectorAdd` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\03-Execution\AsyncVectorAdd\AsyncVectorAdd.csproj --locked-mode
dotnet build .\samples\tutorials\03-Execution\AsyncVectorAdd\AsyncVectorAdd.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\03-Execution\AsyncVectorAdd\AsyncVectorAdd.csproj --configuration Release --no-build -- --arch gfx1100
```
...
The cloud log validates five lengths, 100 lifecycle repeats, expected invalid-ordinal handling, and zero failures.
...
The case log is `logs/AsyncVectorAdd.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `AsyncVectorAdd.csproj` | .NET target and local HIPSharp project reference |
