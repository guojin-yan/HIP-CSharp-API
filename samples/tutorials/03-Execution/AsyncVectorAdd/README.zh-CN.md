...
`AsyncVectorAdd` 是 03-Execution 模块中的教程案例。使用 HIPRTC 编译 VectorAdd，使用两个非阻塞 Stream 和 Event，验证五种长度，捕获预期错误，并重复 100 次生命周期操作。

## 本案例学习什么

本案例重点演示：HIPRTC、异步 H2D/Kernel/D2H 顺序、Event 同步、CPU 比较和确定性释放。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `AsyncVectorAdd` 状态是本案例的
权威云端结果。
...
云端日志验证五种长度、100 次生命周期重复、预期非法序号错误处理，且 failureIndex=-1。

完整证据目录位于
[`Radeon_Cloud/records/20260818-161709-tutorials`](../../../../../Radeon_Cloud/records/20260818-161709-tutorials)。
本案例日志为 `logs/AsyncVectorAdd.log`；`results.csv` 保存 20 个教程的状态和退出码。
...
| `AsyncVectorAdd.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
| `packages.lock.json` | 锁定的依赖图 |
| `../../run-cloud-verification.sh` | 云端架构检测、构建矩阵、运行和证据保存 |

建议先阅读所在模块说明：[`03-Execution`](../README.zh-CN.md)。
