...
`StreamAndEvent` 是 03-Execution 模块中的教程案例。在一个非阻塞 Stream 上排队异步 H2D/D2H Copy，记录 Event、同步并验证字节。

## 本案例学习什么

本案例重点演示：HipStream、HipEvent、异步顺序、同步和资源生命周期。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `StreamAndEvent` 状态是本案例的
权威云端结果。
...
本案例日志为 `logs/StreamAndEvent.log`；`results.csv` 保存 20 个教程的状态和退出码。
...
| `StreamAndEvent.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
