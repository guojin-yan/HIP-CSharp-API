# StreamAndEvent / Stream and Event

<p align="center"><strong>English</strong> | <a href="README.zh-CN.md">简体中文</a></p>

`StreamAndEvent` is the 03-Execution tutorial case. Queues asynchronous H2D and D2H copies on one non-blocking stream, records an event, synchronizes, and verifies the bytes.

## What This Teaches

This case focuses on HipStream, HipEvent, asynchronous ordering, synchronization, and resource lifetime.
...
```bash
dotnet run --project samples/tutorials/03-Execution/StreamAndEvent/StreamAndEvent.csproj --configuration Release
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`StreamAndEvent` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\03-Execution\StreamAndEvent\StreamAndEvent.csproj --locked-mode
dotnet build .\samples\tutorials\03-Execution\StreamAndEvent\StreamAndEvent.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\03-Execution\StreamAndEvent\StreamAndEvent.csproj --configuration Release --no-build --
```
...
The cloud log reports a passing asynchronous memory round trip.
...
The case log is `logs/StreamAndEvent.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `StreamAndEvent.csproj` | .NET target and local HIPSharp project reference |
