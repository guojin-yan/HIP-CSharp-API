...
`GraphCaptureReplay` 是 05-Graph 模块中的教程案例。捕获异步内存往返，实例化 Graph、重放并验证结果。

## 本案例学习什么

本案例重点演示：HipRuntime.CaptureGraph、HipGraphExec、Stream 捕获边界和受控 NotSupported 处理。
...
需要架构的案例请把 `gfx1100` 替换成 `rocminfo` 输出的实际值。矩阵中的 `GraphCaptureReplay` 状态是本案例的
权威云端结果。
...
| `GraphCaptureReplay.csproj` | .NET 目标框架和本地 HIPSharp 项目引用 |
| `packages.lock.json` | 锁定的依赖图 |
| `../../run-cloud-verification.sh` | 云端架构检测、构建矩阵、运行和证据保存 |

建议先阅读所在模块说明：[`05-Graph`](../README.zh-CN.md)。
