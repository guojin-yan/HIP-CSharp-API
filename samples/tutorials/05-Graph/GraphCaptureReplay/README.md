# GraphCaptureReplay / graph capture and replay

<p align="center"><strong>English</strong> | <a href="README.zh-CN.md">简体中文</a></p>

`GraphCaptureReplay` is the 05-Graph tutorial case. Captures an asynchronous memory round trip, instantiates the graph, replays it, and verifies the result.

## What This Teaches

This case focuses on HipRuntime.CaptureGraph, HipGraphExec, stream capture boundaries, and controlled NotSupported handling.
...
```bash
dotnet run --project samples/tutorials/05-Graph/GraphCaptureReplay/GraphCaptureReplay.csproj --configuration Release
```

For cases that need an architecture, replace `gfx1100` with the value reported by `rocminfo`. The
`GraphCaptureReplay` entry in the matrix is the authoritative cloud status for this tutorial.
...
cd HIP-CSharp-API
dotnet restore .\samples\tutorials\05-Graph\GraphCaptureReplay\GraphCaptureReplay.csproj --locked-mode
dotnet build .\samples\tutorials\05-Graph\GraphCaptureReplay\GraphCaptureReplay.csproj --configuration Release --no-restore
dotnet run --project .\samples\tutorials\05-Graph\GraphCaptureReplay\GraphCaptureReplay.csproj --configuration Release --no-build --
```
...
The cloud log reports a successful captured graph replay.
...
The case log is `logs/GraphCaptureReplay.log`; `results.csv` records the status and exit code for all 20 tutorials.
...
| `GraphCaptureReplay.csproj` | .NET target and local HIPSharp project reference |
